...
public class WifiSurveyRecordLogger extends SurveyRecordLogger implements IWifiSurveyRecordListener
{
    ...
    {
        if (!loggingEnabled || wifiBeaconRecords.isEmpty()) return;

        final List<WifiRecordWrapper> records = new ArrayList<>(wifiBeaconRecords);
        ...
        handler.post(() -> {
            synchronized (geoPackageLock)
            {
                ...
                {
                    Timber.e(e, "Something went wrong when trying to write Wi-Fi survey records");
                }
            }
        });
    }
    ...
    /**
     * Writes a single 802.11 Beacon Record to the GeoPackage log file using the provided FeatureDao.
     * <p>
     * This method must only be called while holding the {@link #geoPackageLock}.
     *
     * @param featureDao        The FeatureDao to use for writing.
     * @param wifiRecordWrapper The 802.11 Beacon Record to write.
     */
    private void writeWifiBeaconRecord(FeatureDao featureDao, WifiRecordWrapper wifiRecordWrapper)
    {
