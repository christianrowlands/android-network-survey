package com.craxiom.networksurvey.gpstest.ui.gnss.data
...
import com.craxiom.networksurvey.gpstest.util.SatelliteUtils
...
class SharedSensorManager(
    ...
    private val _sensorUpdates = callbackFlow {
        ...
        Timber.d("Starting sensor updates")
        ...
        try {
            if (SatelliteUtils.isRotationVectorSensorSupported(context)) {
                ...
                Timber.e("Device doesn't support sensor TYPE_ROTATION_VECTOR or TYPE_ORIENTATION")
                close()
            }
        } catch (e: Exception) {
            Timber.e(e, "Exception in sensor flow")
            close(e) // in case of exception, close the Flow
        }

        awaitClose {
            Timber.d("Stopping sensor updates")
    ...
    private fun initMagField() {
        Timber.d("Initializing Mag Field...")
        ...
        if (!::geomagneticField.isInitialized && lastLocation != null) {
            ...
            Timber.d("Mag Field initialized")
        }
    }
    ...
    private fun maybeTruncateVector(event: SensorEvent) {
        if (!truncateVector) {
            try {
                ...
                Timber.e(e, "Samsung device error? Will truncate vectors")
                truncateVector = true
